...
namespace ivhd::facade
{
	FacadeCasterAdam::FacadeCasterAdam(std::shared_ptr<core::Core> core)
		: FacadeCaster(core)
		, m_internalCaster(std::make_shared<ivhd::embed::cast::ivhd::CasterAdam>(core->system()))
	{
	}
	...
	void FacadeCasterAdam::calculatePositions(IParticleSystem& ps)
	{
		try
		{
			...
			dynamic_cast<ivhd::embed::cast::ivhd::CasterAdam*>(m_internalCaster.get())->calculatePositions(facadePs->internalSystem());
		}
	...
	void FacadeCasterAdam::calculateForces(IParticleSystem & ps, IGraph & graph)
	{
		try
		{
			...
			float energy = 0.1f;
			dynamic_cast<ivhd::embed::cast::ivhd::CasterAdam*>(m_internalCaster.get())->calculateForces(energy, facadePs->internalSystem(), facadeGraph->internalGraph());
		}
		catch (std::exception & ex)
		{
			m_ext_core->logger().logWarning("Failed to cast data using CasterAdadelta.calculatePositions. Error message: " + *ex.what());
