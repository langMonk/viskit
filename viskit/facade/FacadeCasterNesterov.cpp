...
namespace viskit::facade
{
	...
	void FacadeCasterNesterov::calculateForces(IParticleSystem& ps, IGraph& graph)
	{
		try
		{
			...
			dynamic_cast<embed::cast::ivhd::CasterNesterov*>(m_internalCaster.get())->calculateForces(energy, facadePs->internalSystem(), facadeGraph->internalGraph(), interactions);
		}
		catch (std::exception & ex)
		{
