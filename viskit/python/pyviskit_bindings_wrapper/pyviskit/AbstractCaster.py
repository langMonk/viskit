...
class AbstractCaster:
    ...
    def __init__(self,
        ...
        self.caster_random = self.resource_factory.createCaster(bindings.CasterType.Random, bindings.OptimizerType.NONE)
        self.parser = self.resource_factory.createParser(parser_type)
        self.random_graph_generator = self.resource_factory.createGraphGenerator(graph_generator_type)
    ...
    def fit_transform(self, dataset_file_path, labels_file_path, graph_file_path, output_file_path):
        ...
        self.random_graph_generator.generate(self.particle_system, self.graph, random_neighbors_count,
                                             self.get_arg_or_default(BINARY_DISTANCES))
        ...
        for _ in range(self.get_arg_or_default(ITERATIONS)):
            self.viskit.computeCastingStep(self.particle_system, self.graph, self.caster)

        if self.get_arg_or_default(REVERSE_NEIGHBORS_STEPS) > 0:
            reverse_graph_generator = self.resource_factory.createGraphGenerator(bindings.GraphGeneratorType.Reverse)
            reverse_graph_generator.generate(self.particle_system, self.graph, self.graph_helper)
            self.graph.removeRandomNeighbors()
            self.random_graph_generator.generate(self.particle_system, self.graph, self.graph_helper,
                                                 random_neighbors_count, self.get_arg_or_default(BINARY_DISTANCES))
        ...
        for _ in range(self.get_arg_or_default(L1_STEPS)):
            self.viskit.computeCastingSteps(self.particle_system, self.graph, self.caster)
