...
Random::Random(core::System& system)
...
void Random::generate(particles::ParticleSystem& ps, graph::Graph& graph, graph::Graph& graphHelper, size_t k, bool distancesEqualOne)
{
    m_ext_system.logger().logInfo("[kNN Generator] Determining random neighbors using helper graph...");
    ...
    for (size_t i = 0; i < ps.countParticles(); i++) {
        for (auto random = 0; random < k; random++) {
            while (true) {
                const auto j = math::randInt(0, ps.countParticles() - 1);
                if (j != i) {
                    if (!alreadyNeighbors(i, j, graphHelper)) {
                        auto distance = 1.0f;
                        if (!distancesEqualOne) {
                            distance = ps.vectorDistance(i, j);
                        }
