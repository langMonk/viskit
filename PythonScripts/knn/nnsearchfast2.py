...
sth = int(sqrt(X.shape[0]))
index_flat = faiss.IndexIVFFlat(quantizer, M, sth)
...
with open(sys.argv[2], "wb") as binary_file:
    ...
    binary_file.write(N.to_bytes(8, byteorder="little"))
    binary_file.write((N*k).to_bytes(8, byteorder="little"))
    for i in range(0, N):
        skipped = False
